...
# Test script for file validation - verifies that missing files are detected correctly
# This test intentionally creates a scenario where files are missing and validates error handling

set -e  # Exit on any error

# Source the logging function from deploy.sh
source ./deploy.sh
...
run_validation_test() {
    ...
    test_log "Expected error code: $expected_error_code"
...
if run_validation_test "Missing secrets.yaml" "secrets.yaml" "115"; then
...
if run_validation_test "Missing deployment.yaml" "deployment.yaml" "116"; then
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    FAILED_TESTS=$((FAILED_TESTS + 1))
fi
...
if run_validation_test "Missing service.yaml" "service.yaml" "117"; then
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    FAILED_TESTS=$((FAILED_TESTS + 1))
fi
...
if run_validation_test "Missing ingress.yaml" "ingress.yaml" "118"; then
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
    FAILED_TESTS=$((FAILED_TESTS + 1))
fi
