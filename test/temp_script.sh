...
push_image_to_registry() {
    ...
    while [ $registry_verification_attempts -lt $max_registry_verification_attempts ]; do
        ...
        if registry_response=$(curl -s "http://localhost:32000/v2/my-ag-ui-app/tags/list" 2>/dev/null); then
            ...
            log_warning "Registry catalog request failed on attempt $registry_verification_attempts at $(date '+%Y-%m-%d %H:%M:%S')"
        fi
        
        # If not the last attempt, wait with exponential backoff delay
...
log_info "Starting Docker image push process at $(date '+%Y-%m-%d %H:%M:%S')"

# Execute the image push function
if ! push_image_to_registry; then
    log_error "ERROR: Docker image push to microk8s registry failed at $(date '+%Y-%m-%d %H:%M:%S')"
...
log_info "Docker image push to microk8s registry completed successfully at $(date '+%Y-%m-%d %H:%M:%S')"
log_info "=== DOCKER IMAGE PUSH TO MICROK8S REGISTRY: COMPLETED ==="
exit 0
