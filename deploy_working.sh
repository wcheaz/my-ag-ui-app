...
log "Fixing microk8s permissions..."
multipass exec "$VM_NAME" -- sudo usermod -a -G microk8s ubuntu
multipass exec "$VM_NAME" -- sudo mkdir -p /home/ubuntu/.kube
multipass exec "$VM_NAME" -- sudo chown -R ubuntu:ubuntu /home/ubuntu/.kube || true
log "SUCCESS: microk8s permissions fixed"

# Wait a bit for group changes to take effect
log "Waiting for group changes to take effect..."
sleep 5
...
for addon in dns storage ingress; do
    ...
    if ! multipass exec "$VM_NAME" -- sudo microk8s enable "$addon"; then
...
log ""
log "Next Steps:"
log "  1. Verify deployment: multipass exec $VM_NAME -- sudo microk8s kubectl get all"
log "  2. Access VM: multipass shell $VM_NAME"
log "  3. Check microk8s status: multipass exec $VM_NAME -- sudo microk8s status"
log "=========================================================="
